$(document).ready(function() {
	...
	$("#redeemFromBtn").click(function(){
		...
		}  else if(host=='api.bitcore.io_bitcoincash'){
	...
	/* retrieve unspent data from api.bitcore.io for bitcoin cash */
	function listUnspentBlockdozer_bitcoincash(redeem) {

		$.ajax ({
			type: "GET",
			cache: false,
			url: "https://api.bitcore.io/api/BCH/mainnet/address/"+redeem.addr+"/?unspent=true",
			dataType: "json",
			error: function(data) {
				$("#redeemFromStatus").removeClass('hidden').html('<span class="glyphicon glyphicon-exclamation-sign"></span> Unexpected error, unable to retrieve unspent outputs!');
			},
			success: function(data) {
				...
				if((data[0].address && data[0].txid) && data[0].address==redeem.addr){
					$("#redeemFromAddress").removeClass('hidden').html('<span class="glyphicon glyphicon-info-sign"></span> Retrieved unspent inputs from address <a href="https://api.bitcore.io/api/BCH/mainnet/address/'+redeem.addr+'" target="_blank">'+redeem.addr+'</a>');
					for(var i in data){
						var o = data[i];
